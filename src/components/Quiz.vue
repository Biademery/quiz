<template>
    <body>
      oi
      <h1>
        F <span style="color: red">.</span> R <span style="color: aqua">.</span> I
        <span style="color: yellow">.</span> E <span style="color: red">.</span> N
        <span style="color: yellow">.</span> D <span style="color: aqua">.</span> S
      </h1>
  
      <div class="result py-4 d-none bglight text-center">
        <div class="container lead">
          <p>
            You got
            <span class="percentColor display-4 p-3">0%</span>
            of the quiz right!
          </p>
        </div>
      </div>
  
        <form action="" class="d-flex p-2 flex-column">
          <ol>
            <li>
              <h3>Where do Rachel and Ross go on their first date?</h3>
              <div>
                <input type="radio" name="question1" id="question1a" value="A" />
                <label for="question1a">Central Perk</label>
              </div>
              <div>
                <input type="radio" name="question1" id="question1b" value="B" />
                <label for="question1b">Ross's place</label>
              </div>
              <div>
                <input type="radio" name="question1" id="question1c" value="C" />
                <label for="question1c">Cinema</label>
              </div>
              <div>
                <input type="radio" name="question1" id="question1d" value="D" />
                <label for="question1d">Planetarium</label>
              </div>
              <div>
                <input type="radio" name="question1" id="question1e" value="E" />
                <label for="question1e">Central Park</label>
              </div>
              <p class="result first"></p>
            </li>
            <li>
              <h3>How many categories Monica organises her towels?</h3>
              <div>
                <input type="radio" name="question2" id="question2a" value="A" />
                <label for="question2a">7</label>
              </div>
              <div>
                <input type="radio" name="question2" id="question2b" value="B" />
                <label for="question2b">10</label>
              </div>
              <div>
                <input type="radio" name="question2" id="question2c" value="C" />
                <label for="questio2c">5</label>
              </div>
              <div>
                <input type="radio" name="question2" id="question2d" value="D" />
                <label for="question2d">11</label>
              </div>
              <div>
                <input type="radio" name="question2" id="question2e" value="E" />
                <label for="question2e">15</label>
              </div>
              <p class="result second"></p>
            </li>
            <li>
              <h3>In the season 6 Thanksgiving episode, what dessert does Rachel try and make?</h3>
              <div>
                <input type="radio" name="question3" id="question3a" value="A" />
                <label for="question3a">Cheesecake</label>
              </div>
              <div>
                <input type="radio" name="question3" id="question3b" value="B" />
                <label for="question3b">Trifle</label>
              </div>
              <div>
                <input type="radio" name="question3" id="question3c" value="C" />
                <label for="question3c">Chocolate cake</label>
              </div>
              <div>
                <input type="radio" name="question3" id="question3d" value="D" />
                <label for="question3d">Treacle tart</label>
              </div>
              <div>
                <input type="radio" name="question3" id="question3e" value="E" />
                <label for="question3e">Apple crumble</label>
              </div>
              <p class="result third"></p>
            </li>
            <li>
              <h3>How many sisters does Joey have?</h3>
              <div>
                <input type="radio" name="question4" id="question4a" value="A" />
                <label for="question4a">2</label>
              </div>
              <div>
                <input type="radio" name="question4" id="question4b" value="B" />
                <label for="question4b">1</label>
              </div>
              <div>
                <input type="radio" name="question4" id="question4c" value="C" />
                <label for="question4c">4</label>
              </div>
              <div>
                <input type="radio" name="question4" id="question4d" value="D" />
                <label for="question4d">7</label>
              </div>
              <div>
                <input type="radio" name="question4" id="question4e" value="E" />
                <label for="question4e">He doesn't have any sisters</label>
              </div>
              <p class="result fourth"></p>
            </li>
            <li>
              <h3>What is Phoebe's middle name?</h3>
              <div>
                <input type="radio" name="question5" id="question5a" value="A" />
                <label for="question5a">Ursula</label>
              </div>
              <div>
                <input type="radio" name="question5" id="question5b" value="B" />
                <label for="question5b">Steven</label>
              </div>
              <div>
                <input type="radio" name="question5" id="question5c" value="C" />
                <label for="question5c">Muriel</label>
              </div>
              <div>
                <input type="radio" name="question5" id="question5d" value="D" />
                <label for="question5d">Grace</label>
              </div>
              <div>
                <input type="radio" name="question5" id="question5e" value="E" />
                <label for="question5e">She never finds out</label>
              </div>
              <p class="result fifth"></p>
            </li>
            <li>
              <h3>Which Caribbean country does Ross's conference take place on in season 9?</h3>
              <div>
                <input type="radio" name="question6" id="question6a" value="A" />
                <label for="question6a">Jamaica</label>
              </div>
              <div>
                <input type="radio" name="question6" id="question6b" value="B" />
                <label for="question6b">Trinidad</label>
              </div>
              <div>
                <input type="radio" name="question6" id="question6c" value="C" />
                <label for="question6c">Barbados</label>
              </div>
              <div>
                <input type="radio" name="question6" id="question6d" value="D" />
                <label for="question6d">Aruba</label>
              </div>
              <div>
                <input type="radio" name="question6" id="question6e" value="E" />
                <label for="question6e">Grece</label>
              </div>
              <p class="result sixth"></p>
            </li>
            <li>
              <h3>When is Chandler's birthday?</h3>
              <div>
                <input type="radio" name="question7" id="question7a" value="A" />
                <label for="question7a">3th January</label>
              </div>
              <div>
                <input type="radio" name="question7" id="question7b" value="B" />
                <label for="question7b">6th Octuber</label>
              </div>
              <div>
                <input type="radio" name="question7" id="question7c" value="C" />
                <label for="question7c">8th April</label>
              </div>
              <div>
                <input type="radio" name="question7" id="question7d" value="D" />
                <label for="question7d">9th December</label>
              </div>
              <div>
                <input type="radio" name="question7" id="question7e" value="E" />
                <label for="question7e">15th July</label>
              </div>
              <p class="result seventh"></p>
            </li>
            <li>
              <h3>What is Rachel's dad's job?</h3>
              <div>
                <input type="radio" name="question8" id="question8a" value="A" />
                <label for="question8a">Doctor</label>
              </div>
              <div>
                <input type="radio" name="question8" id="question8b" value="B" />
                <label for="question8b">Lawyer</label>
              </div>
              <div>
                <input type="radio" name="question8" id="question8c" value="C" />
                <label for="question8c">Dentist</label>
              </div>
              <div>
                <input type="radio" name="question8" id="question8d" value="D" />
                <label for="question8d">Accountent</label>
              </div>
              <div>
                <input type="radio" name="question8" id="question8e" value="E" />
                <label for="question8e">Enginner</label>
              </div>
              <p class="result eighth"></p>
            </li>
            <li>
              <h3>What's the name of the high school that three of the Friends attended?</h3>
              <div>
                <input type="radio" name="question9" id="question9a" value="A" />
                <label for="question9a">Washington High School</label>
              </div>
              <div>
                <input type="radio" name="question9" id="question9b" value="B" />
                <label for="question9b">Kennedy High School</label>
              </div>
              <div>
                <input type="radio" name="question9" id="question9c" value="C" />
                <label for="question9c">Lincoln High School</label>
              </div>
              <div>
                <input type="radio" name="question9" id="question9d" value="D" />
                <label for="question9d">Bush High School</label>
              </div>
              <div>
                <input type="radio" name="question9" id="question9e" value="E" />
                <label for="question9e">East High School</label>
              </div>
              <p class="result ninth"></p>
            </li>
            <li>
              <h3>What does Phoebe's ex-husband do for a job?</h3>
              <div>
                <input type="radio" name="question10" id="question10a" value="A" />
                <label for="question10a">Vet</label>
              </div>
              <div>
                <input type="radio" name="question10" id="question10b" value="B" />
                <label for="question10b">Hand model</label>
              </div>
              <div>
                <input type="radio" name="question10" id="question10c" value="C" />
                <label for="question10c">Scients</label>
              </div>
              <div>
                <input type="radio" name="question10" id="question10d" value="D" />
                <label for="question10d">Ice skater</label>
              </div>
              <div>
                <input type="radio" name="question10" id="question10e" value="E" />
                <label for="question10e">Fireman</label>
              </div>
              <p class="result tenth"></p>
            </li>
          </ol>
    
          <button type="submit">Submit</button>
        </form>
    </body>
  </template>
  
<script>
  export default {
      name: 'quiz',
      data: []
  }
</script>